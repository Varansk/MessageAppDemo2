...
{
    public class PictureMessageLastMessageConverter : ILastMessageConverter
    {
        public LastMessage Convert(MessageBase Lastmessage)
        {
            ...
            if (Lastmessage is not PictureMessage)
            {
                throw new ArgumentException("Wrong Type");
            }
            else
            {
                return new LastMessage(IconResources.image.ToBitmapImage(), "Image : " + (Lastmessage as PictureMessage).Text);
            }
        }
    }
}
