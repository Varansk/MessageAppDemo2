...
{
    public class VideoMessageLastMessageConverter : ILastMessageConverter
    {
        public LastMessage Convert(MessageBase Lastmessage)
        {
            ...
            if (Lastmessage is not VideoMessage)
            {
                throw new ArgumentException("Wrong Type");
            }
            else
            {
                return new LastMessage(IconResources.play_button.ToBitmapImage(), "Video: " + (Lastmessage as VideoMessage).Text);
            }
        }
    }
}
